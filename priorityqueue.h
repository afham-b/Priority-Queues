...
public:
	...
	T pop ();
  void change_priority(T val, size_t new_priority);
...
template <typename T> 
void PriorityQueue<T>::change_priority(T val, size_t new_priority) { 
  for( int i = 0; i<capacity; i++) {    //sequential search goes through entries array for string 
    if(entries[i].value != val){        // when it finds the array 
      if ( entries[i-1].priority > new_priority)  { //if its ancestors have high priority number
          int j = i; 
          while (entries[j-1].priority > new_priority ){ //percolate up 
            swap(entries[j], entries[j-1]); 
            j--; }                                       //keep percolating up 
           } 
     if ( entries[i+1].priority < new_priority ){     //if descendatns have lower priority number 
        int k= i; 
        while (entries[k+1].priority < new_priority){   //percolate down 
              swap(entries[k], entries[k+1]);           //keep percolating down 
              k++;  
          }
        }
      break;      //if specified string is found, break from sequential search. 
    }
  } 
}